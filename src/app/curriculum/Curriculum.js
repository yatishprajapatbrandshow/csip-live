...
function Curriculum() {
  ...
  const fetchCurriculumDetails = async () => {
    ...
    // Only append corporate_id if it's defined
    const APIURL = `${API_URL_LOCAL}curriculum/details?curriculum_sid=${curriId}`;
        ...
        <h5 className="text-sm text-gray-700 mb-3">{CurriculumData?.name}</h5>
        {
          groupsData.map((item, index) =>
            <ul className=' border-b-[1px] border-[#F9DAEA] mb-5' key={index}>
              ...
              <h4 className='mb-3'>{item?.groupName}</h4>
              {
                item?.topics.map((ele, idx) =>
                  <li className="list-none grid grid-cols-3 gap-10 border-t border-[#F9DAEA] py-3">
                    {/* Column for Check and Topic Name */}
                    <div className="flex items-center gap-5 col-span-1">
                      <div className="w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center">
                        <Check className="w-4 h-4 text-gray-500" />
                      </div>
                      <p className="text-gray-700 font-medium">{ele?.topic}</p>
                    </div>
                    ...
                    <div className="flex items-center justify-center col-span-1">
                      <button className="flex items-center bg-[#9779FF] text-white py-2 px-4 rounded-lg hover:bg-[#8A6CE0] transition-colors duration-200">
