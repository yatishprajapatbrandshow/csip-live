...
import { setOrderID } from '../../redux/actions/orderIdSlice';
...
const landing = ({ initialData, success }) => {
    ...
    const handleApply = async () => {
        try {
            const register = await Registration(initialData?.data, userData)
            ...
            if (register && register.status) {
                ...
                OrderDetNew = register?.data?.orderId;
                dispatch(setOrderID(register?.data?.orderId));
    ...
    const OrderCreate = async () => {
        ...
        try {
            const order = await CreateOrder(initialData?.data, userData)
    ...
    const initiatePayment = async () => {
        ...
        try {
            const Payment = await triggerRazorpay(initialData?.data, userData)
            if (Payment) {
                console.log("step 3")
    ...
    const handleCreatePayment = async (PayData) => {
        ...
        const payload = {
            ...
            activityId: initialData?.data?.sid,
            orderid: OrderDetNew,
            trans_date: Date.now(),
        ...
        loader ? <Loader /> :
            <>
                ...
                <section>
                    <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-4 pb-16 pt-20 lg:pt-10 grid grid-cols-12 gap-5">
                        <div className='col-span-8'>
                            ...
                            <div className="mt-10 flex justify-start gap-x-6">
                                ...
                                {showPopup && <PopUp onClose={() => { setShowPopup(false) }} activity={initialData?.data} onSuccess={handleApply} />}
                    ...
                    <div className='grid grid-cols-2 max-w-[1400px] mx-auto gap-5 '>
                        <div className='mx-auto bg-purple-200 rounded-2xl px-5'>
                            <div className="py-10">
                                <h3 className='font-montserrat2 tracking-tighter text-5xl'>Objective</h3>
                                <p className='mt-5 font-montserrat text-lg' dangerouslySetInnerHTML={{ __html: objective }} />
                            </div>
                        </div>
                        <div className='mx-auto bg-blue-200 rounded-2xl px-5'>
                            <div className="py-10">
                                <h3 className='font-montserrat2 tracking-tighter text-5xl'>{short_name}</h3>
                                <div className="mt-5 font-montserrat text-sm " dangerouslySetInnerHTML={{ __html: short_desc }} />
                            </div>
                        </div>
                    </div>
